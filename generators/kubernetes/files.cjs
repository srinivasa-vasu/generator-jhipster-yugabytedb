...
const { NO, ELASTICSEARCH, GATEWAY, MONOLITH, JWT, PROMETHEUS, CONSUL, EUREKA } = Types;
  ...
  return {
    writeDeployments() {
      for (let i = 0; i < this.appConfigs.length; i++) {
        ...
        if (this.app.prodDatabaseType !== NO && this.kubernetesYBDPRuntime === false) {
          this.template(`db/yql.yml.ejs`, `${appOut}/${appName}-yql.yml`);
          this.template(`db/yql-job.yml.ejs`, `${appOut}/${appName}-yql-job.yml`);
        }
    ...
    writeDistributedSQL() {
      if (this.kubernetesYBDPRuntime) {
        this.template(`db/yql-merge.yml.ejs`, `dsql-${suffix}/yql.yml`);
        this.template(`db/yql-merge-job.yml.ejs`, `dsql-${suffix}/yql-job.yml`);
      }
    },
